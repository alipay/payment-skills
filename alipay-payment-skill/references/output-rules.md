...
3. 内部状态（如支付查询 shortUrl）只能来自当前步骤对应命令的本次输出。
4. 禁止从上下文、历史输出、钱包授权/开通输出、订单页面、收银台链接、旧二维码路径、文件系统搜索结果或诊断日志中补全当前命令未返回的 URL 或图片。
5. `submit-payment` 失败或未返回本次 shortUrl 时，必须原样输出失败内容并停止；禁止把钱包授权短链（如 `https://u.alipay.com/...` / `https://u.alipay.cn/...`）伪造成付款链接或查询链接，禁止补发历史二维码。
